...
The more flexibility the user is given in this choice, the stronger a planning method is. \todoc{Add reference to the intro to Mausam's book. But where and what can be said?} An interesting source that is also devoted to this question is \cite{mausam} and provides AI techniques, which as of today lack theoretical guarantees.
...
Another interesting approach is to consider the dual linear program which solves for the stationary distribution of the optimal policy. The idea in \cite{yasin} is to introduce linear function approximation in the dual LP, thereby approximating the stationary distributions using linear combination of basis functions. Under suitable assumptions it is \cite{yasin} shown that by solving the approximate dual LP results in a policy that competes with the best amongst the class of policies induced naturally by the linear parameterization. The authors in \cite{yasin} propose a stochastic gradient descent algorithm to compute the near-optimal solution of the associated Lagrangian with large Lagrangian coefficients. However, the result depends on ability to choose sampling distributions to control the variance due to the SGD.
\todoc{Yasin's paper should be cited here as an interesting alternative approach. My notes about his paper:
...
\begin{axis}[]
\addplot[smooth,very thick, mark=diamond, each nth point=100] plot file {./V_LP};
\addplot[dashed,very thick, mark=+, each nth point=100] plot file {./V_pol_CS};
\addplot[dotted,very thick,mark=square, each nth point=100] plot file {./V_pol_cone};
\addlegendentry{$J^*$};
...
\begin{tikzpicture}[scale=1,font=\Large]
\begin{axis}[]
\addplot[very thick] plot file {./policy};
\addplot[dashed,very thick] plot file {./policy_CS};
\addplot[dotted,very thick] plot file {./policy_cone};
\addlegendentry{$u^*$};
