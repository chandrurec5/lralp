...
be the error of approximating $J^*$ using the basis functions in $\Phi$.
...
\todoc{Petrik's thesis gives an example that shows that the  \citet{ALP} is unimprovable in a worst-case sense.
(He also have cool ideas, ie, enriching the constraint system, of how to improve things by solving a problem
of the same type with more or different constraints.)
We should maybe add to the appendix his lower bound and the proof of the upper bound in some future version.
Furthermore, an interesting question is whether the bound in \cref{cmt2} is similarly ``tight''.}
...
\end{align*}
\end{theorem}
\todoc{We should note that the conditions can be relaxed: The cone condition is not necessary.
Maybe give this version of the result in the appendix.}
\begin{proof}
...
(the idea here is to penalize higher queue lengths and higher service rates). \todoc{Was this used by others; if so,
cite them!}
...
% (i.e., we used $1, s,s^2$ and $s^3$ as basis vectors).
...
$u_{LRA}(s)=\argmin_{a\in A} g_a(s)+\sum_{s'\in S}p_a(s,s')\hat{J}_{e_{s'}}(s')$. \todoc{Did we have one more constraint here than in the CS case? Why?} 
%
...
\addplot[dashed,very thick, mark=o, each nth point=100] plot file {./lralp_exp/V_pol_CS_ideal};
...
\addplot[dashed, very thick, mark=+, each nth point=10] plot file {./lralp_exp/policy_CS_ideal};
